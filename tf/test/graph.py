...
class Net():
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            nex = tf.shape(self.x)[0]
            shape = [BATCH_SIZE]*int(nex/BATCH_SIZE)
            if nex % BATCH_SIZE != 0:
                shape += [nex % BATCH_SIZE]
            xbatches = tf.split(self.x, shape)
            ybatches = tf.split(self.y_, shape)
            ...
            # Training.
            self.loss_func = (
                    tf.reduce_mean(tf.reduce_sum(tf.square(self.y - Y), reduction_indices=[1]))
                    + self.reg_co * tf.reduce_mean([tf.nn.l2_loss(W) for W in Ws + [Wout]]))
